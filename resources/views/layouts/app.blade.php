    ...
    <aside class="hidden lg:flex min-h-screen flex-col border-r border-zinc-200 bg-[#f7f7f6]">
        ...
        <nav class="flex-1 space-y-1 p-4 text-sm">
            ...
            @if(auth()->check() && auth()->user()->isAdmin())
                ...
                <a href="{{ route('admin.activity-logs.index') }}" class="museum-nav-item flex items-center gap-3 {{ request()->routeIs('admin.activity-logs.*') ? 'active' : '' }}">
...
<div id="mobileMenu" class="mobile-nav">
    ...
    <nav class="flex flex-col">
        ...
        @if(auth()->check() && auth()->user()->isAdmin())
            ...
            <a href="{{ route('admin.activity-logs.index') }}" class="mobile-nav-link {{ request()->routeIs('admin.activity-logs.*') ? 'active' : '' }}">Activity Logs</a>
