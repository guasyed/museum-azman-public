...
<body class="museum-shell flex min-h-screen items-center justify-center bg-[#f6f5f4] p-4">
    <div class="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <form method="POST" action="<?php echo e(route('login.perform', [], false)); ?>" class="mt-6 space-y-4">
            <?php echo csrf_field(); ?>
            ...
            <label class="museum-field">
                <span>Password</span>
                <input id="login-password" type="password" name="password" data-password-toggle-bound="1" required>
            </label>

            <label class="inline-flex items-center gap-2 text-sm text-zinc-600">
                <input id="toggle-login-password" type="checkbox" class="h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-400">
